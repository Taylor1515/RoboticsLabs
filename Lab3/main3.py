...
# store previous motor encoder angles for incremental odometry
prev_left_deg = left.angle()
prev_right_deg = right.angle()

def update_odometry():
    global prev_left_deg, prev_right_deg, pose_x, pose_y, pose_theta
    cur_left = left.angle()
    cur_right = right.angle()
    dl_deg = cur_left - prev_left_deg
    dr_deg = cur_right - prev_right_deg
    prev_left_deg = cur_left
    prev_right_deg = cur_right
    dl_mm = wheel_deg_to_mm(dl_deg)
    dr_mm = wheel_deg_to_mm(dr_deg)
    # differential drive kinematics
    ds = (dl_mm + dr_mm) / 2.0
    dtheta = (dr_mm - dl_mm) / AXLE_TRACK_MM  # radians (approx)
    # integrate using midpoint approximation
    if abs(dtheta) < 1e-6:
        # near straight
        ...
        r = ds / dtheta
        cx = pose_x - r * math.sin(pose_theta)
        cy = pose_y + r * math.cos(pose_theta)
        pose_theta += dtheta
        pose_x = cx + r * math.sin(pose_theta)
        pose_y = cy - r * math.cos(pose_theta)
...
while True:
    ...
    # periodic small stop for stability
    wait(DIST_CHECK_MS)
    elapsed += DIST_CHECK_MS

    if elapsed % 500 < DIST_CHECK_MS:
        stop_motors()
        wait(60)
        # quick safety: if ultrasonic reading wildly large, attempt small recovery
        if d > 1200:
            tank_run(-120, -120)
            wait(300)
            stop_motors()
    ...
    if dist_to_hit <= HIT_RETURN_THRESH_MM and total_traveled_since_hit >= MIN_TRAVEL_BEFORE_DETECT_MM and d <= HIT_DETECT_DIST_MM:
        stop_motors()
        ev3.speaker.beep()
        wait(200)
        break
...
def turn_in_place_left(deg=90):
    if gyro:
        ...
        tank_run(-TURN_SPEED, TURN_SPEED)
        while True:
            if abs(right.angle()) >= abs(wheel_deg):
                break
            update_odometry()
            wait(10)
        stop_motors()
...
# simple go-to-goal to (0,0) using current odometry
